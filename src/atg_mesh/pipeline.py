...
def _payload(node, ts, value, unit, assessment, *, rate, accum, source, rng) -> dict:
    ...
    quality = "ok" if (value is not None and math.isfinite(value)) else "out_of_range"
...
def run(*, offline: bool, start: str, end: str, outdir: Path, cache: Path,
    ...
    # ---- validacao ---------------------------------------------------------
    # Regra: payload que nao passa no schema NAO entra na malha. Um no de campo
    # com leitura invalida deve ficar em silencio, nunca transmitir lixo que o
    # gateway interpretaria como risco.
    valid, rejected, errs = [], [], []
    for p in payloads:
        ...
        if res.ok:
            ...
            rejected.append({"device_id": p["device_id"],
                             "timestamp": p["timestamp"],
                             "sensor_value": p["sensor_value"],
                             "errors": res.errors})
            errs.extend(res.errors[:1])
    n_ok, n_bad = len(valid), len(rejected)
    LOG.info("4/6 validacao: %d validos, %d rejeitados (nao transmitidos)",
             n_ok, n_bad)
    if errs:
        LOG.warning("    exemplos de erro: %s", errs[:3])
    if not valid:
        raise RuntimeError("nenhum payload valido - verifique a coleta de dados")
    payloads = valid
    ...
    metrics = {
        ...
        "payloads": {"total": n_ok + n_bad, "valid": n_ok, "rejected": n_bad,
    ...
    with open(outdir / "payloads_rejeitados.jsonl", "w") as f:
        for r in rejected:
