...
def to_c1_bin(p: dict) -> bytes:
    import math
    v = p.get("sensor_value")
    if v is None or not math.isfinite(float(v)):
        raise ValueError(
            f"sensor_value invalido ({v!r}) em {p.get('device_id')} "
            f"@ {p.get('timestamp')}: um no nunca deve transmitir NaN. "
            f"O payload deveria ter sido rejeitado na validacao.")
    stype = p["sensor_type"]
