...
def main() -> None:
    ...
    print("[1/3] GloFAS: localizando a celula do canal principal do Itajai-Acu")
    print("      (5 km de resolucao: a celula mais proxima da regua pode ser")
    print("       um afluente - varremos uma grade 3x3 e pegamos a maior vazao)")
    coord = ingest.find_river_cell()

    print(f"\n[2/3] GloFAS (Copernicus EMS): vazao diaria {args.a} .. {args.b}")
    daily = ingest.fetch_discharge(args.a, args.b, coord)
    ...
    if peak_q < 50:
        print("      AVISO: vazao de pico implausivel para o Itajai-Acu. "
              "A celula GloFAS provavelmente ainda esta num afluente.")

    start, end = ingest.pick_peak_window(daily, days=args.days)
    print(f"\n[3/3] janela do evento selecionada automaticamente: {start} .. {end}")

    (cache / "event_window.json").write_text(json.dumps(
        {"query": [args.a, args.b], "start": start, "end": end,
         "peak_date": peak_d, "peak_discharge_m3s": peak_q,
         "glofas_cell": list(coord)}, indent=2))
