...
# ---------------------------------------------------------------- regressao
def test_nan_nunca_chega_ao_radio():
    """
    Regressao do erro de 12/07: a API devolveu a serie de chuva como null, os
    NaN atravessaram a validacao ate o codec e o pipeline quebrou com
    "cannot convert float NaN to integer". Duas travas agora:
      1. o validador REJEITA o payload;
      2. o codec recusa codificar, com mensagem clara.
    """
    p = json.loads(json.dumps(EXAMPLE))
    p["sensor_value"] = float("nan")

    assert not parse_and_validate(json.dumps(p)).ok, "validador deixou NaN passar"

    with pytest.raises(ValueError, match="sensor_value invalido"):
        codec.to_c1_bin(p)
